...
export default App;
```
>여기서 주의할 점이 있는데 ```onClick = {this.scrollBox.scroll}``` 같은 형식도 되지만 첨 렌더링될땐 값이 undefined라서 오류가 생깁니다  
>그래서 화살표 함수를 이용해서 버튼을 누를 때 값을 읽어오게해서 오류가 발생하기 않게 해줄 수 있다  
>이렇게 하면 ref를 이용해 DOM에 직접 참고해서 스크롤바를 조정하는 법을 알아봤습니다
